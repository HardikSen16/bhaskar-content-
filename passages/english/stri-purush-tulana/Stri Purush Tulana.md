---
title: "Stri Purush Tulana"
titleEn: "Stri Purush Tulana"
author: "Tarabai Shinde"
language: en
difficulty: student
tags:
  - history
  - social
reading_time: "4 min"
complexity_score: 7
excerpt: "A critique of gender inequality, questioning the divine and societal biases against women."
reflection_prompts:
  - "How does Shinde challenge the traditional understanding of divine impartiality and justice?"
  - "What specific examples does Shinde use to argue that men exhibit the same 'flaws' attributed to women?"
  - "Analyze Shinde's rhetorical strategy in directly addressing the 'Gods' and men."
student_intro: "This passage is an excerpt from Tarabai Shinde's 'Stri Purush Tulana' (A Comparison of Men and Women), a groundbreaking work of feminist literature in Marathi. Written in 1882, it boldly critiques the patriarchal structures and societal biases that oppressed women in 19th-century India. Shinde directly confronts the perceived superiority of men and questions the divine order that, according to her, has perpetuated women's suffering."
vocabulary:
  - term: "omnipotent"
    meaning: "having unlimited power; able to do anything."
    example: "The ancient myths often depict gods as omnipotent beings."
  - term: "impartial"
    meaning: "treating all rivals or disputants equally; fair and just."
    example: "A judge must remain impartial during a trial."
  - term: "agony"
    meaning: "extreme physical or mental suffering."
    example: "The patient endured agony after the accident."
  - term: "discourse"
    meaning: "written or spoken communication or debate."
    example: "The public discourse on climate change has intensified."
  - term: "witchcraft"
    meaning: "the practice of magic, especially sorcery or spells."
    example: "Accusations of witchcraft led to severe persecution in the past."
  - term: "valiant"
    meaning: "possessing or showing courage or determination."
    example: "The valiant soldier fought bravely on the battlefield."
  - term: "insidious"
    meaning: "proceeding in a gradual, subtle way, but with harmful effects."
    example: "The insidious spread of misinformation can damage public trust."
  - term: "perfidy"
    meaning: "deceitfulness; untrustworthiness."
    example: "His betrayal was an act of profound perfidy."
  - term: "audacity"
    meaning: "a willingness to take bold risks; impudence."
    example: "She had the audacity to question the manager's decision."
scholarly_notes:
  - heading: "Context of 'Stri Purush Tulana'"
    body: "Published in 1882, 'Stri Purush Tulana' is considered one of the earliest feminist texts in India. Tarabai Shinde, a progressive thinker, used sharp wit and logic to challenge the prevailing patriarchal norms and religious justifications for women's subjugation. The work was controversial and ahead of its time, advocating for women's rights and equality."
  - heading: "Rhetorical Device: Direct Address to Gods"
    body: "Shinde's opening rhetorical strategy of directly addressing the 'Gods' serves to immediately challenge the divine authority that is often used to uphold patriarchal structures. By questioning their impartiality and the unequal distribution of happiness and suffering between men and women, she undermines the very foundation of divinely ordained inequality."
  - heading: "Critique of Male Hypocrisy"
    body: "A central theme in the passage is Shinde's accusation of hypocrisy against men. She systematically lists alleged female flaws and then argues that men are equally, if not more, prone to these same vices. This technique aims to dismantle the notion of inherent female inferiority by demonstrating shared human fallibility, particularly among men."
---
Let me ask you something, Gods! You are supposed to be omnipotent and freely accessible to all. You are said to be completely impartial. What does that mean? That you have never been known to be partial. But wasn’t it you who created both men and women? Then why did you grant happiness only to men and brand women with nothing but agony? Your will was done! But the poor women have had to suffer for it down the ages.

One comes across several charges against women both in the written literature and in everyday discourse. But do men not suffer from the same flaws that women are supposed to have? Do men not cheat as women do? Theft, incest, murder, robbery, deception, fraud, swindling of government funds, taking bribes, changing truth to falsehood and falsehood to truth—do men not do any of these? . . .

First, if, as you claim, a woman has more power than a device for witchcraft or black magic, let me ask you, you who are endowed with an intellect far more powerful than hers, what have you not achieved with your intellect? You who have made possible what was believed to be impossible, of what worth can a woman’s power be before your valiant deeds? Of none.

Second, it may be true that women are a whirlpool of suspicion. But that is because they are uneducated and all kinds of doubts inhabit their minds. But even then, it must be borne in mind that their suspicions are usually and necessarily about their own relationships. But if one casts just a fleeting glance at the webs of doubt in your minds, one’s eyes will surely be dazed. Your minds are full of all kinds of treacherous plans. "Let's bluff this moneylender and pocket a thousand rupees from him." Or, "Let's tell that jagirdar such and such a thing and swindle him out of some five hundred rupees." Or, "Let's lie to that officer about that particular case and change his judgment in X's favor." Or, "Let's register those false documents instead of the true ones." Or, "That woman Y, what a coquette she really is! What airs she gives herself! Must corner her one of these days, and see whether some affair with her can be managed. My current affair has begun to bore me. This is the chance to end it once and for all and begin a new one." Such disgusting thoughts never enter a woman's mind. This is, of course, not to say that all the women in the world are as luminous as the sun and as pure as the waters of the Holy Ganges. But even if one takes into account the entire female community in the world, it would be difficult to come across more than 10 percent of them who, like you, are caught in the whirlwind of such insidious perfidies, though not a single one of you is free of them.

Third, women are called the acme of impudence. But does your own species lack this quality in any measure? A judicious comparison would reveal the balance weighted far heavier on your side in this respect. Fourth, women are considered a megapolis of inadvertent acts. But what about you, the dastardly, perfidious, treacherous people that you are? You, who would not hesitate even for a moment in cutting somebody's throat immediately after winning his confidence. Do you never commit such insidious acts? You speak as if you are Holy Temples of Reason! Bravo! Bravo, indeed! You consider yourselves erudite and judicious, don't you? Yet can you find a match, anywhere, at any time, for the perfidious acts that you commit every day? On top of all this, you have the audacity to call yourselves judicious! What can anyone say?
